    ...
    for experiment, result in sorted(results.items()):
        for dataset_name, dataset_result in sorted(result.items()):
            for experiment_group, experiment_group_result in sorted(dataset_result.items()):
                for loss, loss_result in sorted(experiment_group_result.items()):
                    print("Experiment: {}, Dataset: {}, Experiment Group: {}, Loss: {}".format(experiment, dataset_name, experiment_group, loss))
    ...
    for experiment in sorted(os.listdir(RESULTS_DIR)):
        ...
        for dataset_name in sorted(os.listdir(os.path.join(RESULTS_DIR, experiment))):
            ...
            for experiment_group in sorted(os.listdir(os.path.join(RESULTS_DIR, experiment, dataset_name))):
                ...
                for loss in sorted(os.listdir(os.path.join(RESULTS_DIR, experiment, dataset_name, experiment_group))):
                    results[experiment][dataset_name][experiment_group][loss] = {'header': [], 'rows': []}
                    log_paths = get_log_paths(os.path.join(RESULTS_DIR, experiment, dataset_name, experiment_group, loss))
                    for log_path in log_paths:
                        parts = os.path.dirname(log_path.split("{}/{}/{}/{}/".format(experiment, dataset_name, experiment_group, loss))[1]).split("/")
                        if len(results[experiment][dataset_name][experiment_group][loss]['rows']) == 0:
                            results[experiment][dataset_name][experiment_group][loss]['header'] = [row.split("::")[0] for row in parts]
                            results[experiment][dataset_name][experiment_group][loss]['header'].append('Validation Categorical Accuracy')
                            results[experiment][dataset_name][experiment_group][loss]['header'].append('Test Categorical Accuracy')
                        results[experiment][dataset_name][experiment_group][loss]['rows'].append([row.split("::")[1] for row in parts])

                        for log_result in parse_log(log_path):
                            results[experiment][dataset_name][experiment_group][loss]['rows'][-1].append(log_result)
