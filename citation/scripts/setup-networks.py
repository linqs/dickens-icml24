...
def build_model(data, config, out_dir, hyperparameters, tuning_hyperparameters):
    ...
    for index in range(NUM_RANDOM_SEEDS):
        ...
        print("Starting: %s -- Current Run: %d -- Seed: %d -- Max Validation Accuracy: %f"
              % (config['dataset'], index, seed, max_validation_accuracy))
    ...
    for partition in ['train', 'test', 'valid', 'latent']:
        ...
        util.write_psl_data_file(os.path.join(out_dir, "deep-predictions-%s.txt" % partition), deep_predictions_data)
        util.write_psl_data_file(os.path.join(out_dir, "deep-probabilities-%s.txt" % partition), deep_probaility_data)
